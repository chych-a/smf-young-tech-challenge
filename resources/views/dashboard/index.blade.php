...
@section('title', 'Dashboard | SMF OCR Panel')
...
@section('content')
    <section class="header-card">
        ...
        <div class="topbar-actions">
            <a href="{{ route('web.invoices.create') }}" class="btn btn-primary">Upload faktury</a>
            <a href="{{ route('web.invoices.index') }}" class="btn btn-secondary">Zobacz dokumenty</a>
        </div>
    </section>
    ...
    <section class="grid cols-2" style="margin-top: 18px;">
        <article class="card">
            <div class="card-header">
                ...
                <a href="{{ route('web.invoices.index') }}" class="btn btn-secondary btn-sm">Wszystkie</a>
            </div>
                ...
                <div class="table-wrap">
                    <table>
                        ...
                        @foreach ($recentInvoices as $invoice)
                            <tr>
                                <td>{{ $invoice->document_number ?: 'Brak numeru' }}</td>
                                <td>{{ $invoice->contractor?->name ?: 'Nie rozpoznano' }}</td>
                                <td>{{ number_format((float) $invoice->total_amount, 2, ',', ' ') }} {{ $invoice->currency }}</td>
                                <td class="table-actions">
                                    <a class="btn btn-secondary btn-sm" href="{{ route('web.invoices.show', $invoice) }}">Podgląd</a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
        ...
        <article class="card">
            <div class="card-header">
                ...
                <a href="{{ route('web.products.index') }}" class="btn btn-secondary btn-sm">Wszystkie</a>
            </div>
                    ...
                    <table>
                        ...
                        @foreach ($recentProducts as $product)
                            <tr>
                                ...
                                <td class="table-actions">
                                    <a class="btn btn-secondary btn-sm" href="{{ route('web.products.edit', $product) }}">Edytuj</a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
